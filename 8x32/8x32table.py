...
def sub_mix(state):
    ...
    for i in range(4):
        X[i] = (state[4*i] << 24) or (state[4*i+1] << 16) or (state[4*i+2] << 8) or (state[4*i+3])
...
def Tei(i, x):
    ...
    y = aes_gmult(a[i%4], Tei_subbytes(x))
...
def submix_dc(state):
    ...
    for k in range(4):
        Y[k] = (Tei(0, ((X[k]>>24) & 0xff))) ^ (Tei(1,((X[k]>>16) & 0xff))) ^ (Tei(2, ((X[k]>>8) & 0xff))) ^ (Tei(3,(X[k] & 0xff)))

    for i in range(4):
        state[4*i] = (Y[i] << 24) & 0xff
        state[4*i+1] = (Y[i] << 16) & 0xff
        state[4*i+2] = (Y[i] << 8) & 0xff
        state[4*i+3] = (Y[i]) & 0xff
...
def aes_cipher(input, output, w):
    ...
    for r in range(1, Nr):
        ...
        state = submix_dc(state)
        for i in range(4):
            print("{0:02x} {1:02x} {2:02x} {3:02x}".format(state[4*i+0], state[4*i+1], state[4*i+2], state[4*i+3]), end=' ')
            print('\n')
