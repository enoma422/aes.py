...
def T_e4():
    ...
    for i in range(256):
        tmp = s_box(i)
        res.append((tmp << 24) ^ (tmp << 16) ^ (tmp << 8) ^ (tmp))
    return res
...
def state2byte(state):
    b = []
    for i in range(4):
        b.append(state[i] & 0xff000000)
        b.append(state[i] & 0x00ff0000)
        b.append(state[i] & 0x0000ff00)
        b.append(state[i] & 0x000000ff)
    return b

def aes32_print_state(state):
    for i in range(4):
        print(hex(state[i]))
    print()
...
def SubWord(w):
    return (s_box(w>>24) << 24) ^ (s_box((w>>16) & 0xff) << 16) ^ (s_box((w>>8) & 0xff) << 8) ^(s_box(w & 0xff))

def AES32_Enc_KeySchedule(k, rk):
    
    for i in range(4):
        rk[0][i] = (k[4*i+0] << 24) ^ (k[4*i+1] << 16) ^ (k[4*i+2] << 8) ^ (k[4*i+3])
    ...
    for i in range(1, 11):
        ...
        for k in range(4):
            if k == 0:
                tmp = SubWord(RotWord(tmp)) ^ Rcon[rcon_cnt]
                rcon_cnt += 1
            rk[i][k] = rk[i-1][k] ^ tmp
            tmp = rk[i][k]
...
def AES8_KeySchedule(k, rk):
    rk = AES32_Enc_KeySchedule(k, rk)
...
rk32 = AES32_Enc_KeySchedule(key, rk32)

aes_out = AES32_enc(aes_in, rk32, aes_out)

print(" PlainText : ")
print(aes32_print_state(aes_in))
print(" Cipher Text : ")
print(aes32_print_state(aes_out))
