...
def s_box(x):
    return (Te4[x] >> 24) & 0xff

def byte2state(b):
    state = [0 for _ in range(4)]
    for i in range(4):
        state[i] = (b[4*i+0] << 24) ^ (b[4*i+1] << 16) ^ (b[4*i+2] << 8) ^ (b[4*i+3])
    return state
...
def state2byte(state):
    ...
    for i in range(4):
        b[4*i] = (state[i] >> 24)
        b[4*i+1] = (state[i] >> 16) & 0xff
        b[4*i+2] = (state[i] >> 8) & 0xff
        b[4*i+3] = (state[i]) & 0xff
        
    return b

def AES32_round(state, rk):
    tmp = [0 for _ in range(4)]

    for k in range(4):
        tmp[k] = Te0[state[k] >> 24] ^ Te1[(state[(k+1)%4] >> 16) & 0xff] ^ Te2[(state[(k+2)%4] >> 8) & 0xff] ^ Te3[state[(k+3)%4] & 0xff]
        tmp[k] ^= rk[k]
...
def AES32_Inv_round(state, rk):
    ...
    for k in range(4):
        tmp[k] = (Td0[state[k] >> 24]) ^ Td1[(state[(k+3)%4] >> 16) & 0xff] ^ Td2[(state[(k+2)%4] >> 8) & 0xff] ^ Td3[state[(k+1)%4] & 0xff] ^ rk[k]
...
def AES32_enc(plaint, rk, ciphert):
    tmp = [0 for _ in range(4)]

    state = byte2state(plaint)
    ...
    for i in range(1, 10):
        state = AES32_round(state, rk[i])
        
    for i in range(4):
        tmp[i] = (Te4[state[i] >> 24] & 0xff000000) ^ (Te4[(state[(i+1)%4] >> 16) & 0xff] & 0x00ff0000) ^ (Te4[(state[(i+2)%4] >> 8) & 0xff] & 0x0000ff00) ^ (Te4[state[(i+3)%4] & 0xff] & 0x000000ff) ^ rk[10][i]

    ciphert = state2byte(tmp)
    
    return ciphert

def AES32_dec(ciphert, rk, plaint):
    tmp = [0 for _ in range(4)]
    
    state = byte2state(ciphert)
    
    for i in range(4):
        state[i] ^= rk[10][i]
        
    for i in range(9, 0, -1):
        state = AES32_Inv_round(state, rk[i])

    tmp[0] = (Td4[state[0] >> 24] & 0xff000000) ^ (Td4[(state[3] >> 16) & 0xff] & 0x00ff0000) ^ (Td4[(state[2] >> 8) & 0xff] & 0x0000ff00) ^ (Td4[(state[1] & 0xff)] & 0x000000ff)
    tmp[1] = (Td4[state[1] >> 24] & 0xff000000) ^ (Td4[(state[0] >> 16) & 0xff] & 0x00ff0000) ^ (Td4[(state[3] >> 8) & 0xff] & 0x0000ff00) ^ (Td4[(state[2] & 0xff)] & 0x000000ff)
    tmp[2] = (Td4[state[2] >> 24] & 0xff000000) ^ (Td4[(state[1] >> 16) & 0xff] & 0x00ff0000) ^ (Td4[(state[0] >> 8) & 0xff] & 0x0000ff00) ^ (Td4[(state[3] & 0xff)] & 0x000000ff)
    tmp[3] = (Td4[state[3] >> 24] & 0xff000000) ^ (Td4[(state[2] >> 16) & 0xff] & 0x00ff0000) ^ (Td4[(state[1] >> 8) & 0xff] & 0x0000ff00) ^ (Td4[(state[0] & 0xff)] & 0x000000ff)
    
    plaint = state2byte(tmp)
...
def AES32_Enc_KeySchedule(w, rk):
    for j in range(11):
        for i in range(4):
            tmp = (w[4*i+16*j] << 24) ^ (w[(4*i)+(16*j)+1] << 16) ^ (w[(4*i)+(16*j)+2] << 8) ^ (w[(4*i)+(16*j)+3])
            rk[j][i] = tmp
            
    return rk

def AES32_Dec_KeySchedule(rk):
    for i in range(10):
        rk[i][0] = Td0[Te4[(rk[i][0] >> 24)] & 0xff] ^ Td1[Te4[(rk[i][0] >> 16) & 0xff] & 0xff] ^ Td2[Te4[(rk[i][0] >> 8) & 0xff] & 0xff] ^ Td3[Te4[(rk[i][0]) & 0xff] & 0xff]
        rk[i][1] = Td0[Te4[(rk[i][1] >> 24)] & 0xff] ^ Td1[Te4[(rk[i][1] >> 16) & 0xff] & 0xff] ^ Td2[Te4[(rk[i][1] >> 8) & 0xff] & 0xff] ^ Td3[Te4[(rk[i][1]) & 0xff] & 0xff]
        rk[i][2] = Td0[Te4[(rk[i][2] >> 24)] & 0xff] ^ Td1[Te4[(rk[i][2] >> 16) & 0xff] & 0xff] ^ Td2[Te4[(rk[i][2] >> 8) & 0xff] & 0xff] ^ Td3[Te4[(rk[i][2]) & 0xff] & 0xff]
        rk[i][3] = Td0[Te4[(rk[i][3] >> 24)] & 0xff] ^ Td1[Te4[(rk[i][3] >> 16) & 0xff] & 0xff] ^ Td2[Te4[(rk[i][3] >> 8) & 0xff] & 0xff] ^ Td3[Te4[(rk[i][3]) & 0xff] & 0xff]
    return rk
...
#decrk32 = [[0 for col in range(4)] for row in range(11)]
